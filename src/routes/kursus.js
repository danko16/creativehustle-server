...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let review = await Rating.findOne({ where: { course_id, student_id: user.id } });

      if (review) {
        return res.status(400).json(response(400, 'Anda sudah meberikan review kursus ini'));
      }
      ...
      const courseRating = getRatings(ratings);
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const review = await Rating.findOne({ where: { student_id: user.id, course_id } });
      return res.status(200).json(response(200, 'Berhasil Mendapatkan Review', review));
    } catch (error) {
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await transaction.commit();
      return res.status(200).json(response(200, 'Berhasil Mengupdate Review', review));
    } catch (error) {
      await transaction.rollback();
      return res.status(500).json(response(500, 'Internal Server Error!'));
    }
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await Rating.destroy({ where: { student_id: user.id, course_id } });
